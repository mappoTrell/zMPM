...
pub fn Solver(comptime s_base: b.basis) type {
    return struct {
        ...
        pub fn run(e: *m.env, tp: *ThreadPool, alloc: std.mem.Allocator) !u64 {
            ...
            try task_arr.init(c_count * mult * (e.shapes.items.len + 1), alloc);
            defer task_arr.deinit();
            ...
            while (@as(f64, @floatFromInt(t)) <= e.timeEnd / e.timeStep) : (t += 1) {
                ...
                while (i < e.grid.gp.len) : (i += incr) {
                    ...
                    tp.schedule(ThreadPool.Batch.from(task_arr.createTask(
                        resetGrid,
                        .{ e, 0, i, end },
                        &wG,
                        ta_i,
        ...
        pub fn resetGrid(e: *m.env, _: usize, start: u64, end: u64) void {
            // wg.start();
            // defer wg.finish();
            const gp = e.grid.gp;
        ...
        pub fn updateGrid(e: *m.env, _: usize, start: u64, end: u64) void {
            const gp = e.grid.gp;
        ...
        pub fn gridToPoints(e: *m.env, shpN: usize, start: u64, end: u64) void {
            const gp = e.grid.gp;

            var bas = switch (s_base) {
        ...
        pub fn pointsToGrid(e: *m.env, shpN: usize, start: u64, end: u64) void {
            const gp = e.grid.gp;
